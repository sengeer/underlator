...
mod tests {
    use super::*;
    use crate::domain::chat::dto::{ChatData, ChatMessageRole, ChatModelRef};
    use crate::domain::host_error::{HostErrorClass, host_error_class};
    use async_trait::async_trait;
    use std::sync::Mutex;

    #[derive(Default)]
    struct TestChatStore {
        inner: Mutex<std::collections::HashMap<String, ChatData>>,
    }

    #[async_trait]
    impl ChatStore for TestChatStore {
    ...
    fn service(store: Arc<TestChatStore>) -> ChatService {
    ...
    async fn create_then_get_roundtrip() {
        let store = Arc::new(TestChatStore::default());
