...
//!
//! Единственная точка сетевого I/O для исходящих адаптеров.
//! Типы `reqwest` / `hyper` не реэкспортируются. Use-cases сюда не входят.
...
impl HttpClient {
    ...
    async fn unary_attempt(
        ...
        if self.config.trace_bodies
            && let Some(body) = request.body.as_deref()
        {
