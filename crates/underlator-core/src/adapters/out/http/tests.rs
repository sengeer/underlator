...
async fn send_json_or_empty_delete_200_empty_body_is_default_success() {
    ...
    let result: crate::domain::model::dto::UnarySuccess = client
...
fn use_case_modules_keep_port_boundaries() {
    ...
    let catalog = std::fs::read_to_string(src.join("application/catalog.rs")).unwrap();
    assert!(
        !catalog.contains("ollama-models.zwz.workers.dev"),
        "application/catalog.rs не должен содержать library URL"
    );
    let chat = std::fs::read_to_string(src.join("application/chat.rs")).unwrap();
    assert!(
        !chat.contains("std::fs"),
        "application/chat.rs не должен вызывать std::fs"
    );
}
...
fn domain_modules_do_not_import_reqwest_or_hyper() {
    let src = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("src");
    let roots = [src.join("domain"), src.join("ports")];
...
fn http_client_has_no_named_ollama_endpoints() {
    let http_dir = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("src/adapters/out/http");
    let mut files = Vec::new();
