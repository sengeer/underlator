...
use crate::events::InstallStatus;
use crate::model::dto::{
    DEFAULT_PROVIDER_ID, DEFAULT_PROVIDER_URL, GenerateRequest, InstallRequest, ProviderConfig,
    RemoveRequest,
};

use super::{LlmProvider, ProviderFactoryConfig, create_provider};
...
fn provider_sources_do_not_import_reqwest_or_vendor_health() {
    let dir = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("src/provider");
...
fn domain_modules_have_no_use_case_functions() {
    ...
    for rel in ["model/mod.rs", "catalog/mod.rs", "chat/mod.rs"] {
