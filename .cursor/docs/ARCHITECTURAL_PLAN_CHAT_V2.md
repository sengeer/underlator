...
### 2.2 Создание IPC API клиента чатов
**Цель:** Создать общий Chat IPC Electron API клиент для React приложения.

**Задачи:**
- Создать файл `react-app/src/shared/apis/electron/electron.ts` в shared слое с API клиентом для взаимосдействия с эндпоинтами чата, в shared слой нужно положить что бы использовать его в разных частях приложения
- Создать индексный файл `react-app/src/shared/apis/electron/index.ts`
- Создать файл с специфичными типами (если потребуется) `react-app/src/shared/apis/electron/types/electron.ts`
- Провести рефакторинг функций тестирования `react-app/src/widgets/settings/tests/chat-ipc.ts` для использования созданного API клиента

**Ожидаемый результат:** Готовый к дальнейшему использованию в разных компонентах и подключенный к chat-ipc.ts модуль с IPC API клиентом чатов в shared слое.

### 2.3 Создание утилит для работы с контекстом чата
**Цель:** Создать универсальные утилиты для управления контекстом чата с поддержкой суммирования и оптимизации.

**Задачи:**
- Создать файл `react-app/src/shared/lib/hooks/use-model/utils/chat-context.ts` для утилит работы с контекстом
- Реализовать утилиту `buildChatPrompt` для построения промпта с учетом истории сообщений и контекста
- Реализовать утилиту `summarizeContext` для сжатия длинного контекста при превышении лимитов модели
- Реализовать утилиту `formatMessagesForProvider` для универсального форматирования сообщений под различные провайдеры (Ollama, Anthropic, OpenRouter)
- Реализовать утилиту `calculateContextTokens` для подсчета токенов в контексте
- Реализовать утилиту `truncateContext` для обрезки контекста с сохранением важных сообщений
- Реализовать утилиту `extractKeyInformation` для извлечения ключевой информации из сообщений для суммирования
- Создать конфигурацию лимитов токенов для различных провайдеров
- Добавить валидацию контекста и обработку ошибок
- Реализовать кэширование обработанного контекста для оптимизации производительности

**Ожидаемый результат:** Набор универсальных утилит для эффективного управления контекстом чата с поддержкой суммирования и оптимизации под различные провайдеры LLM.

### 2.4 Расширение feature-provider для обработки чата
**Цель:** Интегрировать утилиты контекста в feature-provider и реализовать обработку чата с поддержкой контекстного суммирования.

**Задачи:**
- Модифицировать файл `react-app/src/shared/lib/hooks/use-model/feature-provider.ts` для добавления обработки режима чата
- Создать функцию `handleChat` для обработки запросов в режиме чата с использованием утилит контекста
- Реализовать загрузку контекста предыдущих сообщений через IPC API
- Интегрировать утилиты `buildChatPrompt`, `summarizeContext`, `formatMessagesForProvider` в обработку чата
- Реализовать логику контекстного суммирования при превышении лимитов токенов
- Использовать существующую логику streaming для получения ответов от модели
- Реализовать сохранение ответа в контексте чата с обновлением истории
...
- Добавить обработку ошибок и fallback логику при проблемах с контекстом

**Ожидаемый результат:** Расширенный feature-provider с поддержкой режима чата и интеллектуального управления контекстом.

### 2.5 Cоздание UI компонентов прототипирования фичи чата
**Цель:** Создать группу UI компонентов для тестирования функционала текущей реализации чатов

**Задачи:**
- Доделать уже созданный компонент `react-app/src/widgets/chat/ui/chat.tsx` в monkeytype design стиле реализовав UI чатов и типы `react-app/src/widgets/chat/types/chat.ts`. Реализацию UI, стили кнопки удаления текста, input ввода текста, кнопки остановки / генерации запроса можно посмотреть в компоненте `PdfViewer`
- Доделать уже созданный ранее компонент`ChatMessages` в `react-app/src/widgets/chat/ui/chat-messages.tsx` аналогичнно в дизайн в стиле monkeytype и определить типы `react-app/src/widgets/chat/types/chat-messages.ts`
- Доделать уже созданный ранее компонент `ChatSidebar` в `react-app/src/widgets/chat/ui/chat-sidebar.tsx` в стиле monkeytype design, определить типы `react-app/src/widgets/chat/types/chat-sidebar.ts`. Комопнент `ChatSidebar` нужен для отображения списка чатов и должен иметь функциональности создания новых чатов, переключения между чатами, поиска по чатам, удаления чатов
- Доделать уже созданный ранее компонент `MessageBubble` в `react-app/src/widgets/chat/ui/message-bubble.tsx` в стиле monkeytype design, реализовать его типы `react-app/src/widgets/chat/types/message-bubble.ts`  для отображения отдельных сообщений с поддержкой различных типов сообщений, действий с сообщениями (копирование, редактирование) реализацию копирования текста можно посмотреть в компоненте `PdfViewer`, компонент для рендеринга markdown есть в `react-app/src/shared/ui/markdown-renderer`
- Определить стили в дериктории `react-app/src/widgets/chat/styles/` для каждого компонента, с использованием корневых цветов приложения (--background, --foreground, --main, --accent)
- Пока реализовать без Redux слайса, API клиент находится в `react-app/src/shared/apis/chat-ipc`
- Нужно правильно реализовать логику использования модели с помощью хука useModel в typeUse chat
- Для ленивой загрузки и виртуализации используй useIntersectionObserver

**Ожидаемый результат:** Полный набор компонентов полноценного интерфейса чата для тестирования и предварительного прототипирования фичи.

## Этап 3: Создание Redux слайса для управления чатами

### 3.1 Создание типов для chat slice
**Цель:** Создать типы для управления состоянием чатов в Redux.

**Задачи:**
- Создать файл типов `react-app/src/widgets/chat/types/chat-ipc-slice.ts` с интерфейсами для управления состоянием чатов
- Определить интерфейсы на основе уже готовой логики из `react-app/src/widgets/chat/ui/chat-messages.tsx`, `react-app/src/widgets/chat/ui/chat-sidebar.tsx`, `react-app/src/widgets/chat/ui/chat.tsx`, `react-app/src/widgets/chat/ui/message-bubble.tsx` с полным описанием всех необходимых полей
...
### 3.2 Создание ChatSlice для управления состоянием чатов
**Цель:** Создать Redux slice для централизованного управления состоянием чатов.

**Задачи:**
- Создать Redux slice `react-app/src/widgets/chat/models/chat-ipc-slice.ts` для управления состоянием чатов
...
### 3.3 Создание async thunks для операций с чатами
...
- Обеспечить обработку состояний с соответствующими обновлениями состояния
...
### 3.4 Интеграция ChatSlice с основным store и компонентами чатов
**Цель:** Интегрировать ChatSlice в основную систему управления состоянием приложения и с уже созданными компонентами чатов.
...
- Интегрировать chat-ipc-slice.ts на действия связанные с управлением чатом в таких компонентах как: `react-app/src/widgets/chat/ui/chat-messages.tsx`, `react-app/src/widgets/chat/ui/chat-sidebar.tsx`, `react-app/src/widgets/chat/ui/chat.tsx`, `react-app/src/widgets/chat/ui/message-bubble.tsx`
- Удалить прямые вызовы `react-app/src/shared/apis/chat-ipc/chat-ipc.ts` из комопнентов чата
- Очистить всю рудиментарную логику оставшуюся от предыдущей реализации управления чатом

**Ожидаемый результат:** Полностью интегрированный ChatSlice.
...
**Универсальная архитектура контекста:**
Система управления контекстом спроектирована для поддержки множественных провайдеров LLM (Ollama, Anthropic, OpenRouter) через единый интерфейс. Контекст передается через универсальный IPC `window.electron.model.generate` без привязки к специфичным параметрам провайдеров.

**Контекстное суммирование:**
Реализована механика контекстного суммирования по аналогии с Cursor, где при превышении лимитов токенов происходит автоматическое сжатие истории чата. Система извлекает ключевую информацию из старых сообщений и создает краткое резюме для сохранения контекста.

**Оптимизация производительности:**
- Кэширование обработанного контекста для избежания повторных вычислений
- Ленивая загрузка контекста с поддержкой инкрементальных обновлений
- Интеллектуальная обрезка контекста с сохранением важных сообщений
- Подсчет токенов в реальном времени для предотвращения превышения лимитов

**Структура утилит контекста:**
```
react-app/src/shared/lib/utils/chat-context/
├── index.ts                    # Экспорт всех утилит
├── build-chat-prompt.ts        # Построение промпта с контекстом
├── summarize-context.ts         # Суммирование длинного контекста
├── format-messages.ts          # Форматирование под провайдеров
├── calculate-tokens.ts         # Подсчет токенов
├── truncate-context.ts         # Обрезка контекста
├── extract-key-info.ts         # Извлечение ключевой информации
├── context-cache.ts            # Кэширование контекста
├── provider-config.ts          # Конфигурация лимитов провайдеров
└── types.ts                    # Типы для утилит контекста
```
...
### Проблема: Превышение лимитов токенов контекста
**Решение:** Реализовать интеллектуальное суммирование контекста с извлечением ключевой информации, автоматическую обрезку старых сообщений с сохранением важных, динамическое управление размером контекста в зависимости от провайдера, кэширование обработанного контекста для оптимизации.
...
### Проблема: Совместимость с различными провайдерами LLM
**Решение:** Универсальная система форматирования сообщений под различные провайдеры, конфигурационные файлы с лимитами и особенностями каждого провайдера, адаптивные алгоритмы построения контекста, fallback механизмы при недоступности провайдера.
...
- Универсальная система управления контекстом для множественных провайдеров LLM
- Интеллектуальное контекстное суммирование по аналогии с современными AI-инструментами
- Оптимизированная производительность с кэшированием и ленивой загрузкой
