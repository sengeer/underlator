...
- **WHEN** проверяется граница HTTP-слоя после появления адаптера провайдера
- **THEN** исполняемый `OllamaProvider` MAY существовать, если он использует только унифицированный HTTP-клиент ядра и MUST NOT импортировать `reqwest` или `hyper`
- **AND** в core по-прежнему нет исполняемых use-case функций generate/CRUD чата/выборки каталога
