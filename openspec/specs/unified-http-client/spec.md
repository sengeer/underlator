...
Ядро SHALL предоставлять единый исходящий HTTP-клиент как единственную точку сетевого I/O для будущих провайдеров и use-cases. Клиент MUST поддерживать unary JSON и потоковые ответы.
...
- **AND** модули domain use-cases (`model`, `catalog`, `chat`) MUST NOT импортировать `reqwest` или `hyper` напрямую
...
Этот атом MUST развивать только исходящий HTTP-слой ядра. Он MUST NOT реализовывать LLM-провайдер, use-cases `model`/`catalog`/`chat`, входящие Axum routes, Tauri commands, RAG и изменения React.

#### Scenario: No provider or use-case runtime
- **WHEN** атом 2.1 завершён
- **THEN** в core нет исполняемого `OllamaProvider` и нет функций generate/CRUD чата/выборки каталога как use-cases
- **AND** `underlator-server` и `underlator-tauri` не получают исходящий HTTP к LLM и не получают MVP routes/commands из этого атома
